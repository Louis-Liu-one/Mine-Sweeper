...
class MineHelper(tk.Toplevel):
    ...
    def setup_messages(self):
        ...
        self.grid_legend(
            ...
            (flag, '', None, 'Cell which has been marked as a mine'),
            (empty, '#', None, 'Cell which can be opened'),
            (empty, '!', None, 'Cell which must be mine'),).grid(sticky=W)
        ttk.Separator(self.main_frame, orient=HORIZONTAL).grid(sticky=EW)

        self.grid_messages(
            "Don't know how to play this game? Here are some tips:",
            "First, the number which shows when you click a cell,"
        ...
        self.grid_messages(
            "Second, if you know a cell is a mine by logical reasoning,"
            " you can mark it by clicking.", "Besides, if you know a cell"
            " is not a mine by logical reasoning,"
        ...
        self.grid_messages(
            ...
            "Third, if (8 - n) cells around an n-cell are opened,"
        ...
        self.grid_messages(
            "Fourth, if n cells around an n-cell have been marked"
        ...
        self.grid_messages(
            "These are the four basic skill to play Mine Sweeper."
            " You can explore more laws to become a mine sweeper expert!")
    ...
    def grid_cells(self, cells):
        '''布置演示用的扫雷格子，cells是由tuple(图片，文字)，(文字)，或(图片)组成的二维列表。'''
    ...
    def grid_legend(self, *legends):
        '''图例的布置。*legends是形如tuple(图片,图片上的文本,颜色,描述)构成的参数列表。'''
        frame = tk.Frame(self.main_frame, width=self.small_width)
        tk.Label(
            frame, text='Legend',
...
class HandleHelper(MineHelper):

    def setup_messages(self):
        self.grid_messages("First, click to mark a cell as a mine.")
        self.grid_messages("Second, double click to open a cell.")
        self.grid_messages("Third, click the green 'click' sign first.")
        self.grid_messages("Finally, play the game!")
